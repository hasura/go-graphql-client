...
func TestClient_Query_partialDataWithErrorResponse(t *testing.T) {
	...
	if got, want := err.Error(), "Total: 1\nMessage: Could not resolve to a node with the global id of 'NotExist', Locations: [{Line:10 Column:4}]\n"; got != want {
...
func TestClient_Query_noDataWithErrorResponse(t *testing.T) {
	...
	if got, want := err.Error(), "Total: 1\nMessage: Field 'user' is missing required arguments: login, Locations: [{Line:7 Column:3}]\n"; got != want {
		t.Errorf("got error: %v, want: %v", got, want)
	}
	if q.User.Name != "" {
